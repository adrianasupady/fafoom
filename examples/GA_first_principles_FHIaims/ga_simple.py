...
while len(population) < popsize and cnt < cnt_max:
    ...
    if str3d not in blacklist:
        ...
        str3d.perform_aims(p_file, aims_call, name)
        ...
        cnt += 1
if cnt == cnt_max:
    print_output("The allowed number of trials for building the population has \
    been exceeded. The code terminates.")
    sys.exit(0)
...
def mutate_and_relax(candidate, name, prob_for_mut_torsions,
    ...
    while found is False and cnt < cnt_max:
        ...
        if candidate not in blacklist:
            ...
            candidate.perform_aims(p_file, aims_call, name)
...
while iteration < max_iter:
    ...
    (parent1, parent2, fitness) = selection(population, selection_type,
                                            energy_var, fitness_sum_limit)
    ...
    while param < prob_for_crossing and cnt < cnt_max:
        ...
        for child in child1, child2:
            ...
            for attr in attr_list:
                delattr(child, attr)
    try:
        mutate_and_relax(child1, "child1", prob_for_mut_torsions,
                         prob_for_mut_cistrans, max_mutations_torsions,
                         max_mutations_cistrans, iteration, cnt_max)
        ...
        sys.exit(0)
    try:
        mutate_and_relax(child2, "child2", prob_for_mut_torsions,
                         prob_for_mut_cistrans, max_mutations_torsions,
                         max_mutations_cistrans, iteration, cnt_max)
    except Exception as exc:
        print_output(exc)
    ...
    if iteration >= iter_limit_conv-1:
        ...
        d = abs(min_energy[iteration+1]-min_energy[iteration+1-iter_limit_conv])
        if population[0].energy < energy_wanted or d < energy_diff_conv:
            perform_backup()
